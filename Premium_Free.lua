...
VisualTab:AddSection({"Other Visuals"})
...
spawn(function()
    while task.wait(5) do
        ...
        TeleportTab:UpdateDropdown("Select Player", playerNames, playerNames[1] or "")
    end
end)
...
TeleportTab:AddSection({"Teleport Tools"})

-- Bring Player
TeleportTab:AddButton({
    Name = "Bring Selected Player",
    Callback = function()
        local selected = TeleportTab:GetDropdownValue("Select Player")
        if selected then
...
SettingsTab:AddButton({
    ...
    Callback = function()
        local joinScript = string.format('game:GetService("TeleportService"):Teleport(%s, game:GetService("Players").LocalPlayer)', game.PlaceId)
...
SettingsTab:AddLabel({"Lethal Ape: Rampage Hub"})
SettingsTab:AddLabel({"Made by danno"})
SettingsTab:AddLabel({"Using Redz Hub UI Library"})
SettingsTab:AddLabel({"Version: 2.0.0 Ultimate"})
