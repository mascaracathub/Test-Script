...
L_1_[38] = L_1_[16]:MakeWindow({
	...
	["SubTitle"] = "by discord_betaa discord.gg/wWHxH6ARU",
...
--[[ 
    VALENTINE EVENT 2025 - AUTO FARMING & SHOP
    Insert into "Main" Tab (L_1_[93]["Main"])
]]

-- Section for Valentine Event
L_1_[93]["Main"]:AddSection({
    "Valentine Event 2025"
})

-- Auto Random Event Valentine
L_1_[93]["Main"]:AddToggle({
    ["Name"] = "Auto Random Event Valentine",
    ["Description"] = "Automatically claim random event rewards.",
    ["Default"] = false,
    ["Callback"] = function(L_val_random)
        _G["AutoRandomValentine"] = L_val_random
    end
})
...
task.spawn(function()
    while task.wait(15) do
        pcall(function()
            if _G["AutoRandomValentine"] then
                -- Adjust the remote name and arguments as needed
                local args = { [1] = "ValentineEvent", [2] = "RandomReward" }
...
-- Auto Accept Quest Valentine
L_1_[93]["Main"]:AddToggle({
    ["Name"] = "Auto Accept Quest Valentine",
    ["Description"] = "Automatically accept the quest from the event NPC.",
    ["Default"] = false,
    ["Callback"] = function(L_val_quest)
        _G["AutoQuestValentine"] = L_val_quest
    end
})
...
-- Auto Farm Quest Valentine
L_1_[93]["Main"]:AddToggle({
    ["Name"] = "Auto Farm Quest Valentine",
    ["Description"] = "Automatically farm event mobs and collect quest items.",
    ["Default"] = false,
    ["Callback"] = function(L_val_farm)
        _G["AutoFarmValentine"] = L_val_farm
        if not L_val_farm then
            -- Stop any related loops if necessary
        end
    end
})

task.spawn(function()
    while task.wait(_G["Fast_Delay"] or 0.15) do
        pcall(function()
            if _G["AutoFarmValentine"] and game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible then
                -- !!! IMPORTANT: Change the mob names and coordinates below to match the actual event !!!
                local targetMonster = nil
                for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                    if enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
                        -- Change these names to the correct event mobs
                        if enemy.Name == "Cupid" or enemy.Name == "Valentine Soldier" then 
                            targetMonster = enemy
                            break
                        end
                    end
                ...
                if targetMonster then
                    L_1_[4]["Kill"](targetMonster, true)
                else
                    -- !!! IMPORTANT: Change these coordinates to the correct event farming spot !!!
                    if World3 then
                        _tp(CFrame.new(-12561.1074, 337.896759, -7516.68164)) -- Near Mansion
                    elseif World2 then
                        _tp(CFrame.new(-452.39859, 78.3738403, 601.511353)) -- Kingdom of Rose
                    elseif World1 then
                        _tp(CFrame.new(-1316.23193, 30.0985413, 2687.12036)) -- Orange Town
                    end
                end
            end
        end)
...
L_1_[93]["Main"]:AddButton({
    ["Name"] = "Buy Cupid's Charm (Item A)",
    ["Description"] = "Purchase the first item from the shop.",
    ["Callback"] = function()
        pcall(function()
            -- Adjust the remote name and arguments as needed
            local args = { [1] = "ValentineShop", [2] = "BuyItem1" }
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
        end)
    end
})
...
L_1_[93]["Main"]:AddButton({
    ["Name"] = "Buy Lovely Cape (Item B)",
    ["Description"] = "Purchase the second item from the shop.",
    ["Callback"] = function()
        pcall(function()
            local args = { [1] = "ValentineShop", [2] = "BuyItem2" }
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
        end)
    end
})
...
L_1_[93]["Main"]:AddButton({
    ["Name"] = "Buy Heart Rod (Item C)",
    ["Description"] = "Purchase the third item from the shop.",
    ["Callback"] = function()
        pcall(function()
            local args = { [1] = "ValentineShop", [2] = "BuyItem3" }
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
        end)
    end
})
